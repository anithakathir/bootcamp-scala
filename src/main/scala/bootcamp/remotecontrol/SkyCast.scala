...
case class SkyCast(minChannel: Int, maxChannel: Int, currentChannel: Int, previousChannel: Int, blocked: List[Int],minClicks: Int) {
  ...
  def estimateMinClicks(next: Int): SkyCast = {
    ...
    this.copy(currentChannel = next, previousChannel = current, minClicks = minClicks + clicks)
  }

  def directClicks(currentChannel: Int, nextChannel: Int): (Int,Int) ={
    (currentChannel,nextChannel.toString.length)
  }

  def navigateDown(currentChannel: Int, nextChannel: Int): List[Int] = {
  ...
  def navigateUp(currentChannel: Int, nextChannel: Int): List[Int] = {
    if ( currentChannel <= nextChannel )
  ...
  def getNumOfClicks(sequence: (Int,Int) => List[Int])(currentChannel : Int,nextChannel: Int): (Int, Int) = {
